...
mod tests {
    ...
    /// Integration test that loads the real DE442s kernel.
    /// Skipped if kernel file is not present.
    #[test]
    fn load_de442s_segments() {
        let kernel_path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../../kernels/data/de442s.bsp");

        if !kernel_path.exists() {
            eprintln!("Skipping: kernel not found at {}", kernel_path.display());
            return;
        }

        let kernel = SpkKernel::load(&kernel_path).expect("should load de442s.bsp");
    ...
    /// Evaluate Mars Barycenter relative to SSB at J2000.0.
    #[test]
    fn evaluate_mars_barycenter_at_j2000() {
        ...
        let kernel = SpkKernel::load(&kernel_path).expect("should load");
    ...
    /// Test resolve_to_ssb chain for Mars (499).
    #[test]
