...
class TestQueryState:
    def test_mars_position_at_j2000(self, engine_handles):
        ...
        from ctara_dhruv.ephemeris import query
        from ctara_dhruv.types import QueryRequest
        result = query(engine_handles._ptr, QueryRequest(target=499, observer=0, epoch_tdb_jd=J2000))
    ...
    def test_sun_relative_to_ssb(self, engine_handles):
        ...
        from ctara_dhruv.types import QueryRequest
        import math
        result = query(engine_handles._ptr, QueryRequest(target=10, observer=0, epoch_tdb_jd=J2000))
        assert result.state is not None
        state = result.state
    ...
    def test_earth_distance_from_sun(self, engine_handles):
        ...
        from ctara_dhruv.types import QueryRequest
        import math
        result = query(engine_handles._ptr, QueryRequest(target=399, observer=10, epoch_tdb_jd=J2000))
        assert result.state is not None
        state = result.state
        dist_km = math.sqrt(state.x**2 + state.y**2 + state.z**2)
        au_km = 1.496e8
...
class TestQueryUtc:
    def test_query_utc_spherical(self, engine_handles):
        ...
        from ctara_dhruv.ephemeris import query
        from ctara_dhruv.types import QueryRequest, UtcTime, QUERY_OUTPUT_SPHERICAL
        result = query(
            engine_handles._ptr,
            QueryRequest(
                target=499,
                observer=10,
                frame=1,
                utc_time=UtcTime(2024, 1, 1, 12, 0, 0.0),
                output_mode=QUERY_OUTPUT_SPHERICAL,
            ),
        )
        assert result.spherical_state is not None
        ss = result.spherical_state
        assert 0 <= ss.lon_deg < 360
        assert -90 <= ss.lat_deg <= 90
        assert ss.distance_km > 0

    def test_query_utc_with_utctime(self, engine_handles):
        """UTC query with UtcTime should return valid cartesian state when requested."""
        from ctara_dhruv.ephemeris import query
        from ctara_dhruv.types import QueryRequest, UtcTime
        utc = UtcTime(2024, 1, 1, 12, 0, 0.0)
        result = query(engine_handles._ptr, QueryRequest(target=499, observer=10, utc_time=utc))
        assert result.state is not None
        state = result.state
        assert abs(state.x) > 0
